...
class SplitScreenEditor:
    ...
    def save_file(self):
        if not self.current_filepath: self.current_filepath = filedialog.asksaveasfilename(defaultextension=".py", filetypes=[("Python Files", "*.py")])
        if self.current_filepath:
            with open(self.current_filepath, "w") as f: f.write(self.code_text.get("1.0", tk.END))
            self.root.title(f"Editor — {os.path.basename(self.current_filepath)}")

    def run_code(self):
        self.turtle_screen.clear(); self.turtle_screen.bgcolor("black")
        self.t = turtle.RawTurtle(self.turtle_screen); self.t.hideturtle()
        exec(self.code_text.get("1.0", tk.END), {}, {"t": self.t})

    def show_about_art(self):
        self.turtle_screen.clear(); self.turtle_screen.bgcolor("black")
        w = turtle.RawTurtle(self.turtle_screen); w.hideturtle(); w.speed(0); w.penup()
        w.goto(0, 40); w.color("#6c5ce7"); w.write("TURTLE BENCH", align="center", font=("Consolas", 20, "bold"))
        w.goto(0, 0); w.color("white"); w.write("v1.3.0 Stable Release", align="center", font=("Consolas", 12))
